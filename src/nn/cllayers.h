...
* @Last Modified time: 2017-04-28 18:04:36
*/
...
class Linear : public CLLayer {
  ...
  public:
	...
	void forward() {
		...
		TIMEFUNCTION ( cl_matrix_mult ( y, W, x, false, false, 1.0f, 0.0f ) );

	}

	void backward() {

		TIMEFUNCTION ( cl_matrix_mult ( dW, dy, x, false, true, 1.0f, 0.0f ) );
		TIMEFUNCTION ( cl_matrix_mult ( dx, W, dy, true, false, 1.0f, 0.0f ) );
	...
	virtual void sync_host() {
		...
		TIMEFUNCTION ( W.sync_host() );

	};

	void applyGrads ( float alpha ) {

		TIMEFUNCTION ( cl_elementwise ( W, dW, alpha, "fmad_lmem" ) );
...
class Sigmoid : public CLLayer {
  ...
  public:
	...
	void forward() {
		...
		TIMEFUNCTION (cl_elementwise ( y, x, "logistic" ));

	}

	void backward() {

		TIMEFUNCTION (cl_elementwise ( dx, dy, y, "dlogistic" ));
...
class ReLU : public CLLayer {
  ...
  public:
	...
	void forward() {
		...
		TIMEFUNCTION (cl_elementwise ( y, x, "relu" ));

	}

	void backward() {

		TIMEFUNCTION (cl_elementwise ( dx, dy, y, "drelu" ));
...
class Softmax : public CLLayer {
  ...
  public:
	...
	void forward() {
		...
		TIMEFUNCTION (cl_sub_max_coeff ( x ));
		TIMEFUNCTION (cl_softmax ( y, x ));
	...
	void backward() {
		...
		TIMEFUNCTION ( cl_elementwise ( dx, dy, y, "dsoftmax" ) );
