/*
    ...
    @Last Modified time: 2017-04-28 12:32:15
*/
...
float cross_entropy_host (Eigen::MatrixXf& error, Eigen::MatrixXf& logprobs, Eigen::MatrixXf& predictions, Eigen::MatrixXf& targets) {
	float ce = 0.0f;
	//check what has happened and get information content for that event
	logprobs = - (predictions.unaryExpr (std::ptr_fun (::logf) ).array() );
	error.array() = logprobs.array() * targets.array();
	ce = error.sum();
	return ce;
}
