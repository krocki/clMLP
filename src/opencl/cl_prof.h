...
void show_profiling_data ( Dict<prof_data> &pdata, sort_method_type sort_method = SORT_BY_TIME_DESC, bool profiling_enabled = true ) {
	...
	switch ( sort_method ) {
		...
		case SORT_BY_TIME_DESC:
			sorted_idxs = pdata.sorted_idxs ( [&] ( size_t i1, size_t i2 ) {return pdata.entries[i1].time > pdata.entries[i2].time;} );
			break;
		case SORT_BY_FLOPS_DESC:
			sorted_idxs = pdata.sorted_idxs ( [&] ( size_t i1, size_t i2 ) {return pdata.entries[i1].flops / pdata.entries[i1].time > pdata.entries[i2].flops / pdata.entries[i2].time;} );
			break;
		case SORT_BY_NAME:
			sorted_idxs = pdata.sorted_idxs ( [&] ( size_t i1, size_t i2 ) {return pdata.entries[i1].key < pdata.entries[i2].key;} );
			break;
	...
	if ( profiling_enabled ) {
		...
		std::cout << std::endl;
		std::cout << "Total profiled time: " << 1e-9 * total_cl_time << " s " << "( " << to_string_with_precision ( ( 100.0 * ( long double ) ( 1e-9 * total_cl_time ) / ( long double ) difference ), 7,
				  3 ) << "% ) " << std::endl;
