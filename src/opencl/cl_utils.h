...
std::vector < deviceInfo > clUtils::listDevices ( void ) {

	std::vector < deviceInfo > devices;
	std::vector < cl_dev_info > available_devices = getAllDevices();
	deviceInfo temp;
	...
	for ( unsigned i = 0; i < available_devices.size(); i++ ) {
		...
		temp.type =
		    getFirstWord ( available_devices[i].platform_string ) + std::string ( " OpenCL" );
		temp.typeId = 0;
...
cl_program clUtils::compileProgram ( const char* const kernel_file, cl_context cxContext,
	...
	if ( source ) {
		...
		cpProgram = clCreateProgramWithSource ( cxContext, 1, ( const char** ) &source, &program_length,
		                                        &ciErrNum );
		free ( source );
		...
		if ( ciErrNum != CL_SUCCESS ) {
			...
			ciErrNum =
			    clGetProgramBuildInfo ( cpProgram, d, CL_PROGRAM_BUILD_LOG, log_size, build_log, NULL );
