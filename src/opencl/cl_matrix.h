...
* @Last Modified time: 2017-04-28 17:33:56
*/
...
class cl_matrix {
  ...
  public:
	...
	int setZero(bool wait = false) {
		...
		matrix_ctx->err = clEnqueueFillBuffer ( matrix_ctx->queue(), device_data, &zero, sizeof ( T ), 0, length() * sizeof ( T ), 0, nullptr, &matrix_ctx->cl_events[func_string] );
	...
	int setOnes(bool wait = false) {
		...
		matrix_ctx->err = clEnqueueFillBuffer ( matrix_ctx->queue(), device_data, &one, sizeof ( T ), 0, length() * sizeof ( T ), 0, nullptr, &matrix_ctx->cl_events[func_string]  );
...
int cl_copy_device_to_device ( cl_ctx *ctx, cl_mem &src, cl_mem &dst, size_t offset_src, size_t offset_dst, size_t bytes, bool wait ) {
	...
	ctx->err = clEnqueueCopyBuffer ( ctx->queue(), src, dst, offset_src, offset_dst, bytes, 0, NULL, &ctx->cl_events[event_string] );

	if ( ( !ctx->asynchronous || wait ) || ctx->profiling_enabled ) ctx->get_profiling_data (event_string );

	if ( ctx->err != CL_SUCCESS ) printf ( "clEnqueueCopyBuffer failed with %d - %s\n", ctx->err, oclErrorString ( ctx->err ) );
...
int cl_copy_matrix_to_device ( cl_ctx *ctx, cl_mem device_data, host_matrix<T> &src, bool wait ) {
	...
	ctx->err = clEnqueueWriteBuffer ( ctx->queue(), device_data, CL_TRUE, 0, bytes, src.data(), 0, NULL, &ctx->cl_events[event_string] );

	if ( ( !ctx->asynchronous || wait ) || ctx->profiling_enabled ) ctx->get_profiling_data (event_string );

	if ( ctx->err != CL_SUCCESS ) printf ( "clEnqueueWriteBuffer failed with %d - %s\n", ctx->err, oclErrorString ( ctx->err ) );
...
int cl_copy_matrix_to_host ( cl_ctx *ctx, host_matrix<T> &dst, cl_mem device_data ) {
	...
	if ( ctx->err != CL_SUCCESS ) printf ( "clEnqueueReadBuffer failed with %d - %s\n", ctx->err, oclErrorString ( ctx->err ) );
